...
def package_delivery(total_distance):
    ...
    if initial_color_angle < 0:
        drop_angle = initial_color_angle + 40 #test to work if it lands all the time
    else:
        drop_angle = initial_color_angle - 40
    ...
    move_robot(-DISTANCE_PER_SCANNING*2, 150)
    time.sleep(0.4)
    ...
    remaining = total_distance - DISTANCE_ENTER 
...
def scan_room():
    ...
    try:
        move_robot(-(DISTANCE_ENTER - DISTANCE_PER_SCANNING), 250)
        time.sleep(3)
        while True:
