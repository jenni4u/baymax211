from utils.brick import Motor, BP, wait_ready_sensors, TouchSensor, reset_brick, busy_sleep
import math
import time
import pendulum as pendulum_mvt
import threading
...
def package_delivery(total_distance, delivery_counter):
    ...
    while True:
        if emergency_triggered():
            ...
            pendulum_mvt.emergency_stop_arms()
            return
        if (abs(pendulum_mvt.scanner_motor.get_position() - drop_angle) < 2) :
            break
        busy_sleep(0.01)
    pendulum_mvt.scanner_motor.set_dps(0)
    ...
    # safe_sleep(1.5)
    pendulum_mvt.scanner_motor.set_dps(direction)
    while True:
        if emergency_triggered():
            wheels_stop()
            pendulum_mvt.emergency_stop_arms()
            return
        if (abs(pendulum_mvt.scanner_motor.get_position() - initial_color_angle) < 2) :
            break
        busy_sleep(0.01)
    pendulum_mvt.scanner_motor.set_dps(0)
    #pendulum_mvt.scanner_motor.set_dps(0)
    ...
    print("remaining distance is: " + str(remaining))
    ...
    # while ((time.time() - start_time) < (0.05 + remaining * DISTTODEG / 150)) and not emergency_stop:
    #     busy_sleep(0.01)
